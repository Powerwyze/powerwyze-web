(() => {
 ...
 if(params.get('card')==='metal')f.querySelector('[value="Metal"]').checked=true;
 ...
 function update(){const name=fields.name.value.trim()||'Your name',company=fields.company.value.trim()||'YOUR BUSINESS';const ai=$('#ai-addon').checked;const card=f.querySelector('[name=card_type]:checked').value;const qty=Math.max(1,Number(fields.quantity.value)||1);const amount=(card==='Metal'?50:30)*qty;const monthly=ai?15:5;
