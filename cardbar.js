(() => {
 ...
 function update(){const name=fields.name.value.trim()||'Your name',company=fields.company.value.trim()||'YOUR BUSINESS';const ai=$('#ai-addon').checked;const card=fields.card_type.value;const qty=Math.max(1,Number(fields.quantity.value)||1);const promo=fields.promo_code.value.trim().toLowerCase();const discount=promo==='powercard'?50:0;const amount=50*qty-discount;const monthly=ai?15:5;
 ...
 $('#promo-status').textContent=discount?'Promo applied: one free card ($50 off). Hosting and add-ons are billed separately.':promo?'Promo code not recognized.':'Enter a promo code if you have one.';
 $('#card-total').textContent='$'+amount;$('#monthly-total').textContent='$'+monthly+'/mo';$('#ai-price-line').hidden=!ai;
 const rows=[['Card',card+' × '+qty],['Design',fields.design.value],['Landing page',fields.landing_style.value],['Automations',[...f.querySelectorAll('[name=automations]:checked')].map(i=>i.value).join(', ')||'None selected'],['AI Cardologist',ai?'Requested · $10/month':'Not added'],['Promo code',promo||'None'],['Card discount',discount?'-$50 · One free card':'$0'],['Cards · one-time','$'+amount],['Hosting + add-ons','$'+monthly+'/month']];
